...
Vector2D *leafPostion = new Vector2D(100, 100);
...
void Ant::findLeaf() {
    cout << "Szukam liścia. Moja pozycja: " << this->position->x << " " << this->position->y << endl;
    this->velocity = new Vector2D(leafPostion->x - position->x, leafPostion->y - position->y);

    if(distance(leafPostion, this->position) <= 10) {
        this->brain->popState();
...
void Ant::goHome() {
    cout << "Mam liść, wracam do domu. Moja pozycja: " << this->position->x << " " << this->position->y << endl;
...
void Ant::runAway() {
    cout << "Uciekam! Czyjaś syra! Moja pozycja: " << this->position->x << " " << this->position->y << endl;
...
int main() {
    Ant *obj = new Ant(10, 10);
    while(true) {
        this_thread::sleep_for(chrono::milliseconds(200));
        cout << obj->position->x <<endl;
        obj->update();
    }
